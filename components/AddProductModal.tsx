...
const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onAddProduct, categories }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name || !price || !category || !imageUrl || !stock) {
        addToast({ type: 'error', message: 'Please fill all required fields, including the product image.' });
        return;
    }
    onAddProduct({
        name,
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="flex space-x-4">
                ...
                <div className="flex-1">
                    ...
                    <input type="number" value={stock} onChange={e => setStock(e.target.value)} step="1" min="0" placeholder="e.g., 50" className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500" required/>
                </div>
            </div>
            <div>
          ...
          <div className="p-6 bg-slate-50 flex justify-end space-x-3 rounded-b-lg">
            ...
            <button type="submit" className="px-4 py-2 bg-blue-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-blue-700">Add Product</button>
          </div>
        </form>
      </div>
    </div>
